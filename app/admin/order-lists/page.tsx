...
import { getSalesInvoices } from "@/src/services/admin";
...
export default function OrderListsPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const json = await getSalesInvoices({
          page: 1,
          per_page: 10,
        });

        const mapped: Order[] = json.data.sales_invoices.map((item: any) => ({
          sales_invoice_id: item.name,
